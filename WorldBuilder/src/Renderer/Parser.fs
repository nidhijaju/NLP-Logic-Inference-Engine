...
let partialParse inp =
    match (unWrapOK inp) with
        | hd::tl -> Some (None, Ok tl)
        | _ -> failwithf "shouldn't happen"
...
let rec (|PNOT|_|) inp =
    ...
    | PMATCH Not (Ok (lst, str)) ->
        match lst with
        | Ok _ -> Some (Some NOT, lst)
        | _ -> Some(None, lst)
    | _ -> Some (None, inp)
...
let rec (|PNOUN|_|) inp =
    match inp with
    | Error unparsed -> Some (None, Error unparsed)
    | PNOT (Some not1, PNOUN (Some noun, lstRest)) ->
        match lstRest with
        | Ok _ -> Some (Some (not1 noun), lstRest)
        | _ -> Some (None, lstRest)
    | PMATCH N (Ok (lst, str)) -> 
        match lst with
...
let rec (|PADJ|_|) inp =
    match inp with
    | Error unparsed -> Some (None, Error unparsed)
    | PNOT (Some not1, PADJ (Some adj, lstRest)) ->
        printfn "%A" adj
        match lstRest with
        | Ok _ -> Some (Some (not1 adj), lstRest)
        | _ -> Some (None, lstRest)
    | PMATCH A (Ok (lst, str)) -> 
        match lst with 
...
and (|PIMPL|_|) inp =
    match inp with
    | Error unparsed -> Some (None, Error unparsed)
    | PMATCH I (Ok (PSTDEXP (expopt, lstRest), strI)) ->
        match expopt with
        | Some exp -> 
            match lstRest with
            | Ok _ -> Some (Some (IMPL (strI, exp)), lstRest)
            | _ -> Some (None, lstRest)
        | None -> partialParse inp
    | _ -> Some (None, inp)

and (|PQUANT|_|) inp =
    match inp with
    | Error unparsed -> Some (None, Error unparsed)
    | PMATCH Q (Ok (PSTDEXP (expopt, lstRest), strQ)) ->
        match expopt with
        | Some exp -> 
            match lstRest with
...
and (|PCOND|_|) inp =
    match inp with
    | Error unparsed -> Some (None, Error unparsed)
    | PMATCH C (Ok (PSTDEXP (expopt, (PIMPL (Some impl, lstRest))), strC)) ->
        match expopt with
        | Some exp -> 
            match lstRest with
...
and (|PQUANTR|_|) inp =
    match inp with
    | Error unparsed -> Some (None, Error unparsed)
    | PMATCH QR (Ok (PSTDEXP (expopt, lstRest), strQR)) ->
        match expopt with
        | Some exp -> 
            match lstRest with
            | Ok _ ->  
